  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="flex items-end justify-between mb-6">
        <h2 className="text-3xl font-extrabold tracking-tight">Catalog</h2>
      </div>
        ...
        {products.map(p => {
          ...
          return (
            <Link key={p.id} to={`/product/${p.id}`} className="group bg-white rounded-xl border border-slate-200 hover:border-pink-200 shadow-sm hover:shadow-md transition p-4">
              {p.cover_image_url && <img src={p.cover_image_url} alt={p.title} className="rounded-lg mb-3 h-44 w-full object-cover group-hover:scale-[1.02] transition" />}
